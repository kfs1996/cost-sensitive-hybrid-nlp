...
HYBRID_COMBOS = list(itertools.combinations(ALL_EMBEDDINGS, 2))
...
def evaluate_model(ds_name, algo, combo_name, fold_idx, X_train_hyb, X_test_hyb, y_train, y_test, w_dict, out_file, num_classes):
    ...
    generate_and_save_plots(y_test, preds, y_prob, num_classes, "Phase_3-B", ds_name, combo_name, algo)
...
def main():
    print(f"Loading data for Phase 3-B (2-Way Hybrid Cost-Sensitive Classical ML)...")
    ...
    out_dir = _ROOT / "phase_3_b"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_file = out_dir / "hybrid_csl_ml_reproduction.csv"
    
    if not out_file.exists():
        with open(out_file, "w") as f:
            f.write("dataset,algo,embed,fold,acc\n")
            
    print(f"Total 2-Way Combinations: {len(HYBRID_COMBOS)}")
